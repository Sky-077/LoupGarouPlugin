...
## a rajouter 
- ~~les particules que voient les joueurs bedrock en etant feu follet ou un role qui peut voir l'invisibilite des autres roles invisibles sont trop grosse et rapide~~ → **diagnostiqué comme hors plugin (Geyser)** : réduire la quantité (3→1, offsets divisés par ~2) n'a rien changé au rendu côté Bedrock, confirmant que c'est Geyser qui mappe ce type de particule "effet" (`WITCH`) différemment sur ce client, indépendamment des paramètres envoyés par le plugin — même limitation que la désynchro de mouvement déjà constatée. Remis à une quantité généreuse (5 particules, offsets plus larges) pensée pour un bon rendu côté Java, puisque réduire n'apportait aucun bénéfice côté Bedrock de toute façon. Pas de correctif possible côté plugin pour Bedrock sur ce point précis.
- [ ] À revalider en jeu (client Java) : les particules d'invisibilité (Feu Follet/Petite Fille/Loup-Garou Perfide, `StealthVisionManager`) ont une taille satisfaisante.