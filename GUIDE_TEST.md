...
## a rajouter 
- ~~les particules que voient les joueurs bedrock en etant feu follet ou un role qui peut voir l'invisibilite des autres roles invisibles sont trop grosse et rapide~~ → tentative précédente (réduire la quantité, 3→1, offsets divisés par ~2) sans effet côté Bedrock avec le type `WITCH` (magenta), diagnostiqué comme un mapping Geyser propre à ce type de particule. **Remplacé** par `Particle.ENTITY_EFFECT` (le rendu vanilla du tourbillon de particules coloré d'un effet de potion actif, coloré avec `PotionEffectType.INVISIBILITY.getColor()`) au lieu de `WITCH`, dans l'espoir que ce type standard soit mappé plus uniformément par Geyser — **hypothèse non garantie, à valider en jeu**.
- [ ] À revalider en jeu (client Java **et** Bedrock) : les particules d'invisibilité (Feu Follet/Petite Fille/Loup-Garou Perfide, `StealthVisionManager`) ont désormais l'aspect d'un tourbillon d'effet de potion (plus de violet "witch spell"), une taille/couleur satisfaisante, et une taille cohérente entre Java et Bedrock (vérifier si le changement de type de particule a résolu ou non le problème de rendu trop gros/rapide côté Bedrock).